...
// Import data from Airtable export to Supabase
...
async function importCompanies() {
  ...
  for (const record of data) {
    ...
    companies.push({
      ...
      description: record.fields['Description (from Company Insights)']?.[0] || null,
...
async function importContacts() {
  ...
  for (const record of data) {
    ...
    const companyId = record.fields['Account']?.[0] 
...
async function importSignals() {
  ...
  for (const record of data) {
    ...
    // Map relationships
    const companyId = record.fields['Linked Account']?.[0] 
      ? idMappings.companies[record.fields['Linked Account'][0]] 
      : null;
    
    const contactId = record.fields['Linked Contact']?.[0] 
    ...
    signals.push({
      ...
      impact: record.fields['Signal Impact'] || null,
      sentiment: record.fields['Signal Sentiment'] || null,
      created_at: record.createdTime || new Date().toISOString(),
      updated_at: new Date().toISOString()
    });
  }
...
async function importTasks() {
  ...
  for (const record of data) {
    ...
    // Map relationships
    const companyId = record.fields['Related Account']?.[0] 
    ...
    tasks.push({
      id: newId,
      task_name: record.fields['Task Name'] || '',
      task_type: record.fields['Task Type'] || 'Other',
      status: record.fields['Status'] || 'To Do',
      due_date: record.fields['Due Date'] || null,
      owner: record.fields['Owner'] || null,
      company_id: companyId,
      contact_id: contactId,
      priority: record.fields['Task Priority'] || null,
      suggested_action: record.fields['Suggested Next Action'] || null,
      created_at: record.createdTime || new Date().toISOString(),
      updated_at: new Date().toISOString()
    });
  }
...
async function importResearch() {
  ...
  for (const record of data) {
    const newId = generateUUID();
    idMappings.research[record.id] = newId;
    ...
    research.push({
      id: newId,
      research_id: record.fields['Research ID'] || null,
      company_id: companyId,
      contact_id: contactId,
      topic: record.fields['Topic'] || 'Other',
...
async function importOpportunities() {
  ...
  for (const record of data) {
    ...
    // Map relationships
    const researchId = record.fields['Research Report']?.[0] 
      ? idMappings.research[record.fields['Research Report'][0]] 
      : null;
    
    const companyId = record.fields['Associated Company']?.[0] 
...
async function importInsights() {
  ...
  for (const record of data) {
    const newId = generateUUID();
    idMappings.insights[record.id] = newId;
    ...
    const opportunityId = record.fields['Related Opportunity']?.[0] 
    ...
    insights.push({
      id: newId,
      insight_id: record.fields['Insight ID'] || null,
      type: record.fields['Type'] || 'Other',
      description: record.fields['Description'] || null,
      priority: record.fields['Priority'] || null,
      direct_quotes: record.fields['Direct Quotes'] || null,
      source: record.fields['Source'] || 'Other',
      date: record.fields['Date'] || null,
      research_id: researchId,
      opportunity_id: opportunityId,
      company_id: companyId,
      summary_ai: record.fields['Summary (AI)'] || null,
      tags_ai: record.fields['Tags (AI)'] || null,
      created_at: record.createdTime || new Date().toISOString(),
      updated_at: new Date().toISOString()
    });
  }
